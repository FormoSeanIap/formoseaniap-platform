...
| SEO surface | Every non-article page declares a `rel=canonical` absolute URL and a baseline set of Open Graph + Twitter Card meta tags so link previews on LinkedIn, Slack, Discord, and similar are legible instead of bare URLs. Article detail pages (`/article.html?id=...` and `/engineer/article.html?id=...`) upgrade their canonical URL and OG/Twitter metadata at runtime once the chosen article loads, so each article shows its own title and excerpt in share previews. `site/robots.txt` lives at the main-site root; `site/sitemap.xml` and `site-eng/sitemap.xml` are regenerated by `scripts/build_articles.py` on every build and list every published article URL alongside the static pages so crawlers can discover articles without running JavaScript. The admin dashboards are both `noindex, nofollow` and Disallowed in `robots.txt`. |
