...
| SEO surface | Every non-article page declares a `rel=canonical` absolute URL and a baseline set of Open Graph + Twitter Card meta tags so link previews on LinkedIn, Slack, Discord, and similar are legible instead of bare URLs. Every page also references a shared `summary_large_image` OG card at `/assets/og/default.png` so share previews render with a branded image instead of a plain-text fallback. Article detail pages (`/article.html?id=...` and `/engineer/article.html?id=...`) upgrade their canonical URL and OG/Twitter metadata at runtime once the chosen article loads, so each article shows its own title and excerpt in share previews (the image stays the default). `site/robots.txt` lives at the main-site root; `site/sitemap.xml` and `site-eng/sitemap.xml` are regenerated by `scripts/build_articles.py` on every build and list every published article URL alongside the static pages so crawlers can discover articles without running JavaScript. The admin dashboards are both `noindex, nofollow` and Disallowed in `robots.txt`. |
...
| Browser icons | A minimalist torii-inspired mark ships as `site/favicon.svg` (light theme) and `site/favicon-dark.svg` (dark theme), a multi-resolution `site/favicon.ico` (16/32/48 pixels) for legacy browser fallback, and a 180×180 `site/apple-touch-icon.png` with a cream backdrop for iOS home-screen saves. The engineering section references the same absolute `/favicon.*` URLs, so CloudFront serves them from the main-site bucket through the default cache behavior. Raster variants are regenerated from the SVG by `infra/build/generate_favicons.py` whenever the design changes (developer-side only, not part of CI). |
| Error pages | CloudFront rewrites both 403 and 404 responses from any origin to the branded `site/404.html` page with HTTP status 404 and a 10-second error cache. This covers the S3 `AccessDenied` XML that OAC returns for missing keys, normalising every "not found" to a consistent, styled HTML page. |
